...
export default function EditDirectorPage() {
  ...
  useEffect(() => {
    async function loadDirector() {
      try {
        ...
        if (!response.ok) {
          throw new Error("Unable to load director.")
        }
        ...
        setDateOfBirth(
          director.dateOfBirth ? director.dateOfBirth.slice(0, 10) : ""
        )
      } catch {
        setError("Unable to load director details.")
      } finally {
        setIsLoading(false)
      }
    }
  ...
  async function handleSubmit(e: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error("Unable to update director.")
      }
      ...
      setError("Unable to update director.")
    } finally {
      setIsSaving(false)
    }
  }
    ...
    <main className="relative mb-20 min-h-screen bg-slate-950 px-4 py-8 text-slate-100 sm:px-6 lg:px-8">
      ...
      <div className="mx-auto flex w-full max-w-4xl flex-col gap-8">
        <div className="space-y-2">
          <h1 className="text-4xl font-semibold tracking-tight text-white">Update Director</h1>
        </div>
